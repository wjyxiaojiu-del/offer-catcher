...
- **面试刷题** — 内置前端/AI-Agent/Golang 题库，支持学习状态追踪、笔记、重点标记（`/interview`）
- **模拟面试** — AI 面试官基于岗位 JD 生成面试计划，SSE 流式对话，自动复盘评分（`/interview/mock`）
- **薄弱点分析** — 根据复习频次自动聚合薄弱知识点（`/interview/weak`）
- **简历编辑** — 上传后可视化编辑简历信息，支持增删改查（`/resume/edit`）
...
# 3. 初始化数据库和题库
docker-compose exec offer-catcher npx prisma db push
docker-compose exec offer-catcher npx tsx prisma/seed-interview.ts
```

数据库文件会通过卷映射持久化到 `./data/` 目录。
...
│   │   ├── page.tsx          # 学习仪表盘
│   │   ├── questions/        # 题库列表 + 题目详情
│   │   ├── mock/page.tsx     # AI 模拟面试
│   │   ├── practice/         # 专项练习
│   │   └── weak/page.tsx     # 薄弱点分析
│   ├── resume/edit/page.tsx  # 简历编辑
│   └── api/                  # API 路由（均经鉴权守卫）
...
│   ├── api-guard.ts          # 鉴权 + 限流
│   ├── db.ts                 # Prisma 单例
│   └── interview/            # 面试模块：题库 DB 操作 + AI 模拟面试逻辑
└── prisma/
    ├── schema.prisma         # 数据模型
    ├── interview-questions/  # 内置面试题库（前端/AI-Agent/Golang）
    └── seed-interview.ts     # 题库导入脚本
```
